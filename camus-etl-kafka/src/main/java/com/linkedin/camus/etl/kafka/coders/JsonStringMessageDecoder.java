...
/**
 * MessageDecoder class that will convert the payload into a JSON object, look
 * for a field named 'timestamp', and then set the CamusWrapper's timestamp
 * property to the record's timestamp. If the JSON does not have a timestamp,
 * then System.currentTimeMillis() will be used. This MessageDecoder returns a
...
public class JsonStringMessageDecoder extends MessageDecoder<byte[], String> {
    ...
    public CamusWrapper<String> decode(byte[] payload) {
        ...
        try {
            jsonObject = new JsonParser().parse(payloadString)
                    .getAsJsonObject();
        ...
        if (jsonObject.has(timestampField)) {
            String timestampString = jsonObject.get(timestampField)
                    .getAsString();
            try {
                timestamp = new SimpleDateFormat(timestampFormat).parse(
